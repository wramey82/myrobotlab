...
public class AvrdudeUploader extends Uploader  {
  ...
  throws RunnerException, SerialException {
    ...
    Map<String, String> boardPreferences = Compiler.getBoardPreferences();
    ...
    if (uploadUsing.equals("bootloader")) {
      ...
      if (uploadUsing.indexOf(':') == -1) {
        t = Compiler.getTarget(); // the current target (associated with the board)
      } else {
        String targetName = uploadUsing.substring(0, uploadUsing.indexOf(':'));
        t = Compiler.targetsTable.get(targetName);
  ...
  throws RunnerException, SerialException {
    ...
    commandDownloader.add(
      "-P" + (Compiler.isWindows() ? "\\\\.\\" : "") + Preferences.get("serial.port"));
    commandDownloader.add(
  ...
  public boolean burnBootloader(String targetName, String programmer) throws RunnerException {
    return burnBootloader(getProgrammerCommands(Compiler.targetsTable.get(targetName), programmer));
  }
  ...
  private Collection getProgrammerCommands(Target target, String programmer) {
    ...
    if ("usb".equals(programmerPreferences.get("communication"))) {
      ...
      params.add("-P" + (Compiler.isWindows() ? "\\\\.\\" : "") + Preferences.get("serial.port"));
  ...
  public boolean avrdude(Collection params) throws RunnerException {
    ...
    if (Compiler.isLinux()) {
      ...
      commandDownloader.add("-C" + Compiler.getHardwarePath() + "/tools/avrdude.conf");
    } else {
      commandDownloader.add("-C" + Compiler.getHardwarePath() + "/tools/avr/etc/avrdude.conf");
    }
    ...
    commandDownloader.add("-p" + Compiler.getBoardPreferences().get("build.mcu"));
