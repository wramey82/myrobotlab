 ...
 * @author GroG
 *
 *	The Chassis service is for general movement control.  The criteria for using this service is a 2 motor 
 *	differential drive and parameters of shape / size of platform. It will probably need relative location of
 *	motors and size of wheels.  A switch for metric/imperial units of measure would be nice.  It will also need
 *	feedback interfaces.  Some feedback devices could include webcam tracking, motor or wheel encoders, compass,
 *	gyros, or accelerometers.  Possibly all these feedback devices will resolve into a RelativePosition or AbsolutePosition
 *	interface.
 *
...
public class DifferentialDrive extends Service {
	...
	public enum Directions {LEFT, STOPPED, RIGHT}
	...
	// Lock for "syncing" time vs read write contention - don't want a turning thread spinning too tightly
	 ...
	 * 
	 * startHeadingPID
	 * startDistancePID  ... or one PID two errors?
	 * 
	 * deltaHeading ~= error
	 * 
	 * offset already done
	 * will need a max power value - don't want it going at 100% power to get to anywhere (i think)
	 * 
	...
	public void calibrate ()
	{
		...
		// time and power can fluctuate so there is a graph of power & time per distance
		// we will simplify by making a set time (1 sec?) to calibrate - might want to have an array of power values
		// THERE IS A HUGE difference between at rest and in motion when applying power
		...
		// we are going to do a spin turn for a very short time to get to the correct heading
	...
	public void setTargetHeading (int value) // maintainHeading ?? if PID is operating
	{
		headingTarget = value;
	...
	/*
	 *   setHeading is to be used by feedback mechanisms encoders, hall effect, optical tracking
	 *   It "invokes"  to message the listeners of changed state.
	 */
	public final void setHeading(int value)
	{
		...
		if (((headingCurrent < at) && (at < headingTarget))
...
/*	
	public static void main(String[] args) {
		...
		DifferentialDrive platform = new DifferentialDrive("platform");
