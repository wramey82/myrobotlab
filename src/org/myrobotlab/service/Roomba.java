...
public class Roomba extends Service implements SerialPort {
	...
	public static final float millimetersPerDegree = (float) (wheelbase
			* Math.PI / 360.0);
	...
	public void setLEDs(boolean status_green, boolean status_red, boolean spot,
		...
		byte cmd[] = { (byte) LEDS, (byte) v, (byte) power_color,
	...
	public boolean computeSafetyFault() {
		safetyFault = (sensor_bytes[BUMPSWHEELDROPS] & WHEELDROP_MASK) != 0
				|| sensor_bytes[CLIFFLEFT] == 1
				|| sensor_bytes[CLIFFFRONTLEFT] == 1
				|| sensor_bytes[CLIFFRIGHT] == 1
	...
	public String sensorsAsString() {
		...
		return "bump:" + (bumpLeft() ? "l" : "_") + (bumpRight() ? "r" : "_")
				+ " wheel:" + (wheelDropLeft() ? "l" : "_")
				+ (wheelDropCenter() ? "c" : "_")
				+ (wheelDropLeft() ? "r" : "_") + " wall:"
				+ (wall() ? "Y" : "n") + " cliff:" + (cliffLeft() ? "l" : "_")
				+ (cliffFrontLeft() ? "L" : "_")
				+ (cliffFrontRight() ? "R" : "_") + (cliffRight() ? "r" : "_")
				+ " dirtL:" + dirtLeft() + " dirtR:" + dirtRight() + " vwal:"
				+ virtual_wall() + " motr:" + motor_overcurrents() + " dirt:"
				+ dirt_left() + "," + dirt_right() + " remo:"
				+ hex(remote_opcode()) + " butt:" + hex(buttons()) + " dist:"
				+ distance() + " angl:" + angle() + " chst:" + charging_state()
				+ " volt:" + voltage() + " curr:" + current() + " temp:"
				+ temperature() + " chrg:" + charge() + " capa:" + capacity()
				+ sd;
	}
