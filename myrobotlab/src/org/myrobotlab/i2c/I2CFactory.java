...
public class I2CFactory {
	...
	public static I2CBus getInstance(int busNumber) throws IOException {
		...
		/*
		String architecture = Platform.getArch();
		try {
			String I2CBusType = "org.myrobotlab.i2c.I2CBusProxy";
			if (architecture.equals(Platform.ARCH_ARM)) {
				// raspi
				I2CBusType = "com.pi4j.io.i2c.impl.I2CBusImpl";
			}

			Object[] param = new Object[0];

			Class<?> c;
			c = Class.forName(I2CBusType);
			Class<?>[] paramTypes = new Class[param.length];
			for (int i = 0; i < param.length; ++i) {
				paramTypes[i] = param[i].getClass();
			}
			Constructor<?> mc = c.getConstructor(paramTypes);
			return (I2CBus) mc.newInstance(param);
		} catch (Exception e) {
