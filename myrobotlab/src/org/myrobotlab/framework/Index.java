...
public class Index<T> {
	...
	/*
	public  void load(String path, String propFileName) {
		if (path == null) {
		...
		try {
			...
			for (String key : properties.stringPropertyNames()) {
				root.buildTree(root, key, properties.getProperty(key));
				//IndexNode.buildTree(root, key, properties.getProperty(key));
			}
		} catch (Exception e) {
			log.error("config did not load from location " + configPath);
	...
	*/
	...
	public static Index getInstance(String path) {
		if (instance == null) {
