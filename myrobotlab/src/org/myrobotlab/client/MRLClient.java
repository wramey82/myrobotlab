...
/**
 * MRLClient class used to interface with a remote running instance or
 * MyRobotLab. The instance of MRL must be running a RemoteAdapter service. This
 * class can send and recieve messages from the MRL instance using only a few
 * methods. The following code is an example of creating a client, registering a
 * MRL instance, sending a subscribe message to a TestCatcher service named
 * "catcher01". Whenever catcher01's catchInteger method is invoked a message
 * will be sent back to the MRL client. And finally a "send" which sends a
 * message to trigger the callback event.
 * 
 * MRLClient api = new MRLClient(); Receiver client = new Receiver();
 * 
 * api.register("localhost", 6767, client); api.subscribe("catchInteger",
 * "catcher01", "myMsg", Integer.TYPE); api.send("catcher01", "catchInteger",
 * 5);
 * 
 */
public class MRLClient implements Receiver {
	...
	/**
	 * method which registers with a running MRL instance. It does this by
	 * sending a service directory update to the MRL's RemoteAdapter. From MRL's
	 * perspective it will appear as if this is another MRL instance with a
	 * single service. The bogus service name is controlled by overriding
	 * Receiver.getMyName()
	 * 
	 * Once a MRLClient registers is may send messages and subscribe to events.
	 * 
	 * @param host
	 *            - target host or ip of the running MRL instance's
	 *            RemoteAdapter
	 * @param port
	 *            - target port of the RemoteAdapter
	 * @param client
	 *            - call back interface to recieve messages
	 * @return
	 */
	final public boolean register(String host, int port, Receiver client) {
		...
		try {
			...
			ServiceWrapper sw = new ServiceWrapper(client.getMyName(), null, sdu.serviceEnvironment);
	...
	/**
	 * method to send a messages to a running MRL instances. @see
	 * register(String, int, Receiver) must be called before "send" can be used.
	 * 
	 * @param name
	 *            - name of the service to receive the message (message
	 *            destination)
	 * @param method
	 *            - method to invoke
	 * @param data
	 *            - parameter data for the method
	 * @return
	 */
	...
	public void subscribe(String outMethod, String serviceName, String inMethod, Class<?>... paramTypes) {
		MRLListener listener = new MRLListener(outMethod, getMyName(), inMethod, paramTypes);
	...
	static public String help() {
		return "java -jar MRLClient.jar -host [localhost] -port [6767] -service [myService] -method [doIt] -data \"data1\" \"data2\" \"data3\"... \n"
				+ "host: the name or ip of the instance of MyRobotLab which the message should be sent." + "port: the port number which the foreign MyRobotLab is listening to."
				+ "service: the Service the message is to be sent." + "method: the method to be invoked on the Service" + "data: the method's parameters.";
