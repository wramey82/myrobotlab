...
public class WiiDAR extends Service {
	 ...
	 * software technique ie. pausing the system until a the servo command to
	 * move to a location has propagated to an IREvent. Or, preferably it would
	 * be a hardware event where an motor's encoder data is paired with the
	 * appropriate IREvent.
	 */
	 ...
	 * 
	 * // move servo all the way to the right // increment servo left until
	 * IRData appears servo.moveTo(60); Thread.sleep(1000); lastIRData = null;
	 ...
	 * (lastIRData != null) { invoke("setServoLeftMax", i);
	 * invoke("setIRLeftMax", lastIRData); break; } }
	 * 
	 * 
	 * // right scan for (int i = servoLeftMax; i > servoRightMax; --i) { Point
	 * p = new Point(i, i, RIGHT, System.currentTimeMillis()); servo.moveTo(i);
	...
	/*
	 * public void initialize (Wii wii, Servo servo) { this.wii = wii;
	 * this.servo = servo;
	 * 
