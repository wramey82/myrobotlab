...
public class IKEngine extends Canvas{
  ...
  /**
   * Changes the number of links and sets up data structrues.
   * Waits until all ik computation is done
   * and it is safe to change variables.
  ...
  public void setLinkLength(int link, double length) {
    ...
    // critical section: nothing depends on local vars while this runs
    // **
    if(link<0 || link>=numLinks) return;
  ...
  /**
   * @param gx,gy,gz the x,y,z components of the goal point
   */
  ...
  /**
   * calculates jointLocations[] and jacobian, then updates thetas[]
   * Makes one ik iteration.
   *
   * @param gl the goal point
   */
  ...
  public void calculate(){
	  ...
	  while (done == false) {
		...
		if( Math.abs(endPoint.subtractFrom(goal).elements[0][0]) <= .01 &&
			Math.abs(endPoint.subtractFrom(goal).elements[1][0]) <= .01 ) {
			    done= true;
			  } 
	    moveToGoal(goal);}
  }
