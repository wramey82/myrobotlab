...
public class MemoryWidget {
	...
	public MemoryWidget(final MemoryDisplay memDisplay) {
		...
		tree.addTreeSelectionListener(new TreeSelectionListener() {
			public void valueChanged(TreeSelectionEvent e) {
				...
				// MAKE NOTE - gui nodes can be many to one relation ship (or one to many???) to actual memory nodes
	...
	public NodeGUI put(String parentPath, Node node)
	{
		...
		// FIXME - only NodeGUI's are in the (or should be in) the HashMap - this should be strongly typed !
		...
		for (Map.Entry<String,?> nodeData : node.getNodes().entrySet())
		{
			...
			{				
				if (clazz == OpenCVData.class)
				{
					...
					//log.info("{}",data);
					// adding a "gui" node - to have a place where a user can highlight
					// to "see" the opencv image data in the video widget
					// addLeaf(newChild, o.getKey(), o.getValue().toString());
					// TODO - compartmentalize the following to methods 
					// TODO - type info should be different field - your now converting typ info through name..
					
					// this adds complexity - in that viewing its preferrable to be able to 
					// select on different types, while "real" memory is much more compact
					// String imageKey = String.format("%s.%s", childKey, "image");
				
					NodeGUI images = (NodeGUI)child.get("images");
					log.info("{}",images);
					if (images == null){
						images = new NodeGUI(child, "images");
						model.insertNodeInto(images, child, child.getChildCount());
						// FIXME - nodeMap - probably not necessary if you learn to use
						// the JTree system
						child.put(images);				
					}
					...
					{
						NodeGUI imgDisplay = (NodeGUI)images.get(img.getKey());
						
						if (imgDisplay == null) {
							imgDisplay = new NodeGUI(images, img.getKey());
							model.insertNodeInto(imgDisplay, images, imgDisplay.getChildCount());
							images.put(imgDisplay);
