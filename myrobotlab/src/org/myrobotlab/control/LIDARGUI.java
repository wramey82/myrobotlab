...
public class LIDARGUI extends ServiceGUI implements ListSelectionListener, VideoGUISource {
    ...
    int height = 768;
    int xyScale = 2;
    ArrayList<ArrayList<Integer>> history = new ArrayList<ArrayList<Integer>>();  //A list of lists for holding old data
    ...
    public void displaySweepData(int[] points) {
        ...
        int[] x = new int[points.length]; //Just initializing them to the correct size, values will be overwritten below
        ...
        graph.drawLine(midw, vheight, midw,  vheight-30);
            ...
            for (int i = 0; i < length; ++i) {
                // polar coordinates !
//                x = (int) ((points[i]) * Math.cos(((i * 100 / length) + startingAngle) * (3.14159 / 180)) + xOffset);
//                y = vheight - (int) ((points[i]) * Math.sin(((i * 100 / length) + startingAngle) * (3.14159 / 180)));  //vheight inverts the Y-coordinates
                x[i] = (int) ((points[i]) * Math.cos(((i * 100 / length) + startingAngle) * (3.14159 / 180)));
                y[i] = (int) ((points[i]) * Math.sin(((i * 100 / length) + startingAngle) * (3.14159 / 180)));  //vheight inverts the Y-coordinates             
                System.out.println(x[i] + "\t" + y[i]);
            }


            //map current value to window size.  Values will be plotted so largest value = 95% of the window size
            int[] xMinMax = getMinMax(x);  //Get Max and Min values from the array
            int[] yMinMax = getMinMax(y);  //Get Max and Min values from the array
            System.out.println("xMin= "+xMinMax[0]+" xmax= "+xMinMax[1]+" ymin= "+yMinMax[0]+" ymax= "+yMinMax[1]);
            for (int i = 0; i < length; ++i) {

                //scales the screen so that the values fit within 95% of the X and Y values
//                x[i] = map(x[i], 100, xMax - 100, 0, (int) (vwidth * 0.95))+xOffset/2 ;
//                y[i] = map(y[i], 100, yMax - 100, 0, (int) (vheight * 0.95));
                ...
                */
                ...
                y[i] = vheight - map(y[i], 0, yMinMax[1], 0, vheight-5); //the "vheight -" inverts the data for display in the window
                System.out.println(x[i] + "\t" + y[i]);

//                graph.drawLine(x[i], y[i], x[i], y[i]);//draws a dot on the point
                if(i>0){
                graph.drawLine(x[i-1], y[i-1], x[i], y[i]);//draws an actual line from point to point
                }
            }
            ...
            for (int i = 0; i < history.size(); ++i) {
                ...
                for (int j = sweep.size(); j > 0; j--) {
                    p = sweep.get(j);
////					x = vwidth - ((int) (p * Math.cos((i*100/length)+startingAngle) * zScale));
////                                        y = vheight - ((int) (p * Math.sin((i*100/length)+startingAngle) * zScale));
//                                        x = vwidth ((int) (p * Math.cos((i*100/length)+startingAngle) * zScale)+xOffset);
//                                        y = vheight - ((int) (p * Math.sin((i*100/length)+startingAngle) * zScale));
...
//        log.info("mapping "+ x+ " to a min of "+out_min + " and a max of "+out_max);
