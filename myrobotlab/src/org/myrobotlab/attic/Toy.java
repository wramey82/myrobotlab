...
public class Toy extends Service {
	...
	public void foundToy(CvPoint2D32f trackingPoints[]) {
		if (trackingPoints.length == 3) {
			...
			// graphics.drawString((int)toyCenteroid.x + "," +
			// (int)toyCenteroid.y() + "h " + (int)newHeading,
			// (int)toyCenteroid.x + 10, (int)toyCenteroid.y());

			if (bottomSegment == 0) {
				graphics.drawLine((int) lastToyPosition.position(0).x(), (int) lastToyPosition.position(0).y(), (int) lastToyPosition.position(1).x(), (int) lastToyPosition
						.position(1).y());
				...
				graphics.drawLine((int) lastToyPosition.position(1).x(), (int) lastToyPosition.position(1).y(), (int) lastToyPosition.position(2).x(), (int) lastToyPosition
						.position(2).y());
				graphics.drawLine((int) toyPoint.x(), (int) toyPoint.y(), (int) lastToyPosition.position(0).x(), (int) lastToyPosition.position(0).y());
			} else {
				graphics.drawLine((int) lastToyPosition.position(2).x(), (int) lastToyPosition.position(2).y(), (int) lastToyPosition.position(0).x(), (int) lastToyPosition
						.position(0).y());
				graphics.drawLine((int) toyPoint.x(), (int) toyPoint.y(), (int) lastToyPosition.position(1).x(), (int) lastToyPosition.position(1).y());
			}

			// TODO persist configuration - so appropriate segment is always
			// subsequently chosen &
			// get segment lengths
			int seg0 = (int) (Math.sqrt((trackingPoints[0].x() - trackingPoints[1].x()) * (trackingPoints[0].x() - trackingPoints[1].x())
					+ (trackingPoints[0].y() - trackingPoints[1].y()) * (trackingPoints[0].y() - trackingPoints[1].y())));

			int seg1 = (int) (Math.sqrt((trackingPoints[1].x() - trackingPoints[2].x()) * (trackingPoints[1].x() - trackingPoints[2].x())
					+ (trackingPoints[1].y() - trackingPoints[2].y()) * (trackingPoints[1].y() - trackingPoints[2].y())));

			int seg2 = (int) (Math.sqrt((trackingPoints[2].x() - trackingPoints[0].x()) * (trackingPoints[2].x() - trackingPoints[0].x())
					+ (trackingPoints[2].y() - trackingPoints[0].y()) * (trackingPoints[2].y() - trackingPoints[0].y())));
			...
			if ((int) lastHeading != (int) newHeading) {
				...
				// not quite right - if staying on the same heading you could
				// fail to update position
	...
	public void findThingy(String thingy, int hueMin, int hueMax, int saturationMin, int saturationMax, int valueMin, int valueMax, int minArea, int maxArea) {
		...
		// if the search is successful the name and color will be associated
		// with the target(s) found
		currentlyLookingForName = thingy;
		// yellow 238 253 15 RGB = hex EEFD0F == HSV 45 240 253 = 2DF0FD
		//
		// TODO : WARNING - TOTAL CONFUSION - Camera is BGR so values have to be
		// swapped !
		// int temp = Color.HSBtoRGB((hueMin+hueMax)/2,
		// (saturationMin+saturationMax)/2, (valueMin+valueMax)/2);
		...
		// currentlyLookingForColorAvg = new Color((hueMin+hueMax)/2,
		// (saturationMin+saturationMax)/2, (valueMin+valueMax)/2);
	...
	public void foundThingy(ArrayList<Polygon> polygons) {
		...
		// log.error("found " + polygons.size() + " " +
		// currentlyLookingForName);
		...
		for (int i = 0; i < polygons.size(); ++i) {
			...
			// graphics.drawRect(p.boundingRectangle.x(),
			// p.boundingRectangle.y(), p.boundingRectangle.width,
			// p.boundingRectangle.height);
			if (p.vertices < 8) {
			...
			targets.put(currentlyLookingForName + i, new Target(currentlyLookingForName + " " + i, currentlyLookingForName, x, y, width * height, h, d, p.vertices, p.isConvex,
					currentlyLookingForColorAvg));
		}

		if (polygons.size() > 0) // TODO optimization OpenCV - only send > 0
									// polys
		{
	...
	public void displayTargets() {
		...
		while (it.hasNext()) {
			...
			graphics.drawString(
					"vertices " + t.vertices + " convex " + t.isConvex + " avg color " + t.avgColor.getRed() + " " + t.avgColor.getGreen() + " " + t.avgColor.getBlue(),
					t.centeroid.x + 10, t.centeroid.y + 30);
	...
	public void keyPressed(Integer cmd) {
		...
		case 89: // 'y'
			...
			// findThingy ("yellow blocks", 30, 40, 182, 256, 183, 224, 120,
			// 1600);
			// findThingy ("yellow blocks", 26, 37, 160, 256, 183, 256, 120,
			// 1600); // diff
			// findThingy ("red blocks", 0, 12, 224, 256, 146, 222, 120, 1600);
			// findThingy ("blue blocks", 109, 140, 72, 160, 26, 164, 120,
			// 1600);

			// artifical light - "constant"
			// findThingy ("yellow blocks", 26, 37, 160, 256, 220, 256, 120,
			// 1600);
			// findThingy ("red blocks", 0, 12, 224, 256, 183, 222, 120, 1600);
			// findThingy ("blue blocks", 109, 140, 72, 160, 63, 164, 120,
			// 1600);
			// findThingy ("laptop", 12, 29, 50, 121, 145, 211, 2000, 20600);
			// findThingy ("orange", 12, 19, 228, 256, 198, 241, 110, 1800);

			// thingiesToLookFor.add(new TargetCriteria("orange", 12, 19, 228,
			// 256, 198, 241, 110, 1800));
