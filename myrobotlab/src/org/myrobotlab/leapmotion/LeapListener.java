	...
	public void onFrame(Controller controller) {
		...
		if (frame.hand(hand1).isValid()
			...
			if (pointable.isValid()) {
				...
				// myService.invoke("publishScreenX", new
				// Float(intersect.getX()));
				myService.invoke("publishScreenY", new Float(intersect.getY()));
				myService.invoke("publishInvScreenX",
						new Float(1.0 - intersect.getX()));
				// myService.invoke("publishInvScreenY", new
				// Float(1.0-intersect.getY()));
				// myService.invoke("publishYaw1", new
				// Float(pointable.direction().yaw()));
				// myService.invoke("publishPitch1", new
				// Float(pointable.direction().pitch()));
				// myService.invoke("publishRoll1", new
				// Float(pointable.direction().roll()));
				// myService.invoke("publishX1", new
				// Float(pointable.direction().getX()));
				// myService.invoke("publishY1", new
				// Float(pointable.direction().getY()));
				// myService.invoke("publishZ1", new
				// Float(pointable.direction().getZ()));
		...
		if (!frame.gestures().empty()) {
			for (Gesture g : controller.frame().gestures()) {
				if (g.type()==Gesture.Type.TYPE_KEY_TAP) {
					myService.invoke("keyTap", Boolean.TRUE);
				}
			}
		}
	}
