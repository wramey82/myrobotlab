    ...
    try {
      if (boardsFile.exists()) {
        ...
        for (Object k : boardPreferences.keySet()) {
          ...
          if (!boards.containsKey(board)) boards.put(board, new LinkedHashMap<String,String>());
          (boards.get(board)).put(
            key.substring(key.indexOf('.') + 1),
            (String)boardPreferences.get(key));
        }
      }
    } catch (Exception e) {
    ...
    try {
      if (programmersFile.exists()) {
        ...
        for (Object k : programmerPreferences.keySet()) {
          ...
          if (!programmers.containsKey(programmer)) programmers.put(programmer, new LinkedHashMap<String,String>());
          ( programmers.get(programmer)).put(
            key.substring(key.indexOf('.') + 1),
