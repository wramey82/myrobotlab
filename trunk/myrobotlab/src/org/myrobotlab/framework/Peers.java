...
public class Peers {
	...
	{
		this.name = name;
		//peers.put(getPeerKey(peer), new ServiceReservation(peer, peerType, comment)); ???
	}
	
	/*
	// TODO - question on have to pas original root instance in ????
	public void putAll(Index<ServiceReservation> dna, String peer, String inPeerType, String comment)
	{
		String myKey = getPeerKey(peer);
		String peerType;
		if (!inPeerType.contains("."))
		{
			peerType = String.format("org.myrobotlab.service.%s", inPeerType);
		} else {
			peerType = inPeerType;
		}
		
		// put his type in
//		log.warn(String.format("adding %s %s", myKey, peerType));
		templateDNA.put(myKey, new ServiceReservation(myKey, peerType, comment));
		
		
		// load the dns from it's peers - and put them in
		// FIXME TODO - loadType and recurse put
		// FIXME 
		try {
			Class<?> theClass = Class.forName(peerType);
			...
			for (Entry<String, IndexNode<ServiceReservation>> o : peerRequests.entrySet()) {
				...
				putAll(dna, fullKey, peersr.fullTypeName, peersr.comment);
			}
	...
	*/
	...
	public void put(String peer, String actualName, String peerType, String comment)
	{
		...
		if (reservation == null){
			//log.warn(String.format("templateDNA adding new key %s %s %s %s", fullKey, actualName, peerType, comment));
			templateDNA.put(fullKey, new ServiceReservation(peer, actualName, peerType, comment));
		} else {
			//log.warn(String.format("templateDNA collision - replacing null values !!! %s", peer));			
	...
	// suggestAs will insert only (no update) - but top level inserts bottom won't override !!!
	/**
	 * @param key
	 * @param actualName
	 * @param type
